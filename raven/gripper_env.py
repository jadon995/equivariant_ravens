...
import numpy as np
import cameras
from grippers import Spatula
from raven import pybullet_utils
import utils
...
#TODO change to panda gripper
#UR5_URDF_PATH = 'ur5/ur5.urdf'
UR5_WORKSPACE_URDF_PATH = 'ur5/workspace.urdf'
PLANE_URDF_PATH = 'plane/plane.urdf'
PANDA_URDF_PATH='./raven/panda_gripper_play/panda/hand.urdf'
...
class Environment(gym.Env):
  ...
  def __init__(self,
    ...
    #self.homej = np.array([-1, -0.5, 0.5, -0.5, -0.5, 0]) * np.pi # TODO remove or replace
    ...
    self.action_space = gym.spaces.Dict({
        'pose0':
            gym.spaces.Tuple(
                (self.position_bounds, #pick location
                 gym.spaces.Box(-1.0, 1.0, shape=(4,), dtype=np.float32))), #pick angle
        'pose1':
            gym.spaces.Tuple(
  ...
  def reset(self):
    ...
    #print('env_====================',self.obj_ids)
    self.ee = self.task.ee(PANDA_URDF_PATH,obj_ids=self.obj_ids)
    # Reset end effector.
    self.ee.release() #??
    # Re-enable rendering.
  ...
  def step(self, action=None, extend_secs=0):
    ...
    if action is not None:
      timeout = self.task.primitive(self.ee, self.task_env_name, **action)
  ...
  def set_task(self, task):
    ...
    self.task = task
    self.task_env_name = self.task.task_name

...
import numpy as np
import cameras
import planners       # just for continous planner: doesn't matter
import primitives
import panda_primitives
from grippers import Suction
from panda_gripper_play.panda_gripper import  Gripper
import utils
...
class Task():
  ...
  def __init__(self, continuous = False):
    ...
    if continuous:
      ...
      self.primitive = panda_primitives.PickPlace()
  ...
  def _discrete_oracle(self, env):
    ...
    def act(obs, info):  # pylint: disable=unused-argument
      ...
      #print('hmap',hmap.shape,obj_mask.shape)

      # Unpack next goal step.
      objs, matches, targs, replace, rotations, _, _, _ = self.goals[0]
      # print('objs',objs) #[(object_1_uid,(2*pi,0)),(object2),(object3),....], symetry
      # print('matches',matches) #matches [[1]]
      # print('target',targs) # [(pos1,q1),(pos2,q2)]
      # print(replace,rotations) #false true
      ...
      for i in range(len(objs)):
        ...
        if len(targets_i) > 0:  # pylint: disable=g-explicit-length-test
          ...
          #print('target_xyz',targets_xyz.shape)
        ...
        # Handle ignored objects.
          ...
          nn_dists.append(0)
          nn_targets.append(-1)
      order = np.argsort(nn_dists)[::-1] # indice for decreasing order for nn_dists
      #aa=np.array([0., 1., 4., 2., 3.])
      #print('argsort',np.argsort(aa),np.argsort(aa)[::-1])

      # Filter out matched objects.
      order = [i for i in order if nn_dists[i] > 0]

      pick_mask = None
      #print(np.min(obj_mask),np.max(obj_mask))
      for pick_i in order:
        pick_mask = np.uint8(obj_mask == objs[pick_i][0])
        # TODO by haojie, is there any function in cv2 to get the centerline?
        # TODO by haojie, this is just for block-insertion
        if self.task_name =='block-insertion':
          _pose = p.getBasePositionAndOrientation(objs[pick_i][0])
          _rand = np.random.uniform(0.,1.)
          # center lines along x or y
          #_rand=0.8
          if _rand>0.5:
            _x=0
            _y = np.random.uniform(0.035,0.055)
          else:
            _y= 0
            _x = np.random.uniform(0.035, 0.055)
          _z = 0.04
          ...
          _pixel = utils.xyz_to_pix(_xyz,self.bounds,self.pix_size)
          #print(_pixel)

        if self.task_name =='place-red-in-green':
          _pose = p.getBasePositionAndOrientation(objs[pick_i][0])
          _rand = np.random.uniform(0.,1.)
          # center lines along x or y
          #_rand=0.8
        ...
        if self.task_name =='align-box-corner':
          _pose = p.getBasePositionAndOrientation(objs[pick_i][0])
          _rand = np.random.uniform(0.,1.)
          # center lines along x or y
          #_rand=0.8
        ...
        if self.task_name=='stack-block-pyramid':
          _pose = p.getBasePositionAndOrientation(objs[pick_i][0])
          _rand = np.random.uniform(0., 1.)
          # center lines along x or y
          _rand=0.8
        ...
        if self.task_name =='palletizing-boxes':
          #TODO CAHNGE THE x,y later
          _pose = p.getBasePositionAndOrientation(objs[pick_i][0])
          _rand = np.random.uniform(0.,1.)
          shape = p.getVisualShapeData(objs[pick_i][0])
          #print('======================================================shape',shape[0][3])
          sample_x, sample_y = shape[0][3][0], shape[0][3][1]
          _rotation = False
          if sample_x > sample_y:
            _y = 0
            _x = np.random.uniform(-0.025, 0.025)
          else:
            _x= 0
            _y = np.random.uniform(-0.025, 0.025)
            _rotation =True
          # _x=0.
          # _y=0.
          _z = 0.04
        ...
        if self.task_name =='packing-boxes':
          #TODO CAHNGE THE x,y later
          _pose = p.getBasePositionAndOrientation(objs[pick_i][0])
          _rand = np.random.uniform(0.,1.)
          shape = p.getVisualShapeData(objs[pick_i][0])
          ...
          if sample_x > sample_y:
            _y = 0
            _x = np.random.uniform(-sample_x/2 + 0.005, sample_x/2 - 0.005)
          else:
            _x= 0
            _y = np.random.uniform(-sample_y/2 + 0.005, sample_y/2 - 0.005)
            _rotation =True
          # _x=0.
          # _y=0.
          _z = 0.04
        ...
        if self.task_name =='assembling-kits':
          #TODO CAHNGE THE x,y later
          _pose = p.getBasePositionAndOrientation(objs[pick_i][0])
          _rand = np.random.uniform(0.,1.)
          # center lines along x or y
      ...
      #print('pick prob: ',pick_prob.shape)
      #print('pick pixel: ',pick_pix)
      # TODO by haojie
      # For "deterministic" demonstrations on insertion-easy, use this:
      #pick_pix = (160, 80)
      pick_pix = _pixel
      #Pick position instead of pick pose
      ...
      if self.task_name == 'block-insertion':
        pick_pos = list(pick_pos)
        pick_pos[2] = 0.03 + 0.04 # cube is 0.08X0.03 X 0.04
      ...
      if self.task_name == 'assembling-kits':
        # print('ass')
        pick_pos = list(pick_pos)
        pick_pos[2] = 0.03 + 0.03 # cube is 0.08X0.03 X 0.04
        pick_pos = tuple(pick_pos)
        if _rand >0.5:
          _z_rot = Rotation.from_euler('z', np.pi/2)
        else:
      ...
      if self.task_name == 'palletizing-boxes':
        #print(5)
        #print('palletizing-boxes')
        pick_pos = list(pick_pos)
        pick_pos[2] = 0.03 + 0.04
        pick_pos = tuple(pick_pos)
        if _rotation == True:
          _z_rot = Rotation.from_euler('z', np.pi/2)
        else:
          _z_rot = Rotation.from_euler('z', 0)
        #print('before rotation ',_pose[1])
        pick_pose_ = (np.asarray(pick_pos), (_z_rot*Rotation.from_quat(np.asarray(_pose[1]))).as_quat())
        #print('after rotation',pick_pose_[1])
      ...
      # unit quaternion for calculating theta for place
      ...
      # TODO the key points to understand the transformation matrix
      # place_pose is terms of ??? I didn't understand, need take time to consider those rotation, could use
      # panda hand as example
      ...
      if self.task_name=='stack-block-pyramid' and _rand < 0.5:
        #print('==============================')
        z_rot = Rotation.from_euler('z', 0*-np.pi / 2)
        place_pose = (np.asarray(place_pose[0]), (Rotation.from_quat(np.asarray(place_pose[1]))*z_rot).as_quat())
  ...
  def discretized(self,pose0, pose1):

    # pick pose
    p0_pos, p0_ori = list(pose0[0]),list(pose0[1])
    p0_theta = utils.quatXYZW_to_eulerXYZ(p0_ori)[2]
    p0_theta = (2 * np.pi + p0_theta) % (2 * np.pi)
    if p0_theta >= np.pi:
      p0_theta = p0_theta - np.pi

    #TODO change 36 to 72 later for grasp
    p0_theta_i = p0_theta / (2 * np.pi / 36)
    p0_theta_i = np.int32(np.round(p0_theta_i)) % 18
    p0_theta = p0_theta_i * (2 * np.pi / 36)
    p0_xyzw = utils.eulerXYZ_to_quatXYZW((0, 0, p0_theta))
    p0_pose = (np.asarray(p0_pos),np.asarray(p0_xyzw))

    # place pose
    p1_pos, p1_ori = list(pose1[0]), list(pose1[1])
    p1_theta = -utils.quatXYZW_to_eulerXYZ(p1_ori)[2]
    p1_theta_i = p1_theta / (2 * np.pi / 36)
    p1_theta_i = np.int32(np.round(p1_theta_i)) % 36
    p1_theta = p1_theta_i * (2 * np.pi / 36)
    p1_xyzw = utils.eulerXYZ_to_quatXYZW((0, 0, -p1_theta))
    p1_pose = (np.asarray(p1_pos),np.asarray(p1_xyzw))

    return p0_pose,p1_pose
  ...
  def get_random_pose(self, env, obj_size):
    ...
    # add 5 pixesl to erode size for gripper

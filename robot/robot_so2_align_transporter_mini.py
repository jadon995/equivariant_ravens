...
from networks.so2_equ_attention import Attention
# from so2_align_transport import Transport
from networks.so2_align_transport import Transport
...
class TransporterAgent:
    ...
    def get_sample(self, dataset, augment=True):
        ...
        p1_theta = p1_theta - p0_theta # p1 defines the change in orientation
    ...
    def train(self, dataset, writer=None):
        ...
        # theta_0 = p0_theta % np.pi
        # self._display_sample(np.uint8(img[:, :, :3]), p0, theta_0-np.pi/2, p1, theta_0+p1_theta-np.pi/2)
        # print('img',img.shape)
        # print('p0',p0)
    ...
    def test_visualize(self, obs, act):
        ...
        # p0_pix, p0_theta, p1_pix, p1_theta = self.get_act_label(act)
        self._display_sample(np.uint8(img[:, :, :3]), p0, p0_theta-np.pi/2, 
                                                p1, p0_theta+p1_theta-np.pi/2)
        return
    ...
    def act(self, obs, info=None, goal=None):  # pylint: disable=unused-argument
        ...
        p1_xyzw = utils.eulerXYZ_to_quatXYZW((0, 0, -p1_theta-p0_theta))
    ...
    def _display_sample(self, img, pos0, theta0, pos1, theta1):
        ...
        def add_arrow(center, angle, length=30, color='yellow'):
            ...
            end_x = center[0] + length * np.cos(rad)

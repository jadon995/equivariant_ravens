...
do      
    ...
    do  
        ...
        do
            echo Train Task: ${task}-${n_rotations}-${n_demo}-${seed}/${agent}softh${n_align} --gpu_id ${gpu_id};
            gnome-terminal --wait -- /bin/bash  \
                ...
                python train.py --task ${task} --n_demos ${n_demo} --n_rotations ${n_rotations} \
                    ...
                    --gpu_id ${gpu_id} --logging --seed ${seed} --config_file train-soft.yaml --postfix softh${n_align}; \
            ...
            # do
            echo Test Task: ${task}-${n_rotations}-${n_demo}-${seed}/${agent}softh${n_align} --gpu_id ${gpu_id};
            gnome-terminal --wait -- /bin/bash  \
                -c "source ~/.bashrc; cd ~/workspace/equivariant_ravens; pwd;   \
                ...
                python gripper_test.py --task ${task} --n_demos ${n_demo} --n_rotations ${n_rotations} \
                    ...
                    --seed ${seed} --entire --config_file train-soft.yaml --postfix softh${n_align} --disp # --n_steps ${n_tests}; \

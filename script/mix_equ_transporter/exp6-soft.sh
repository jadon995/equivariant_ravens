...
do
    ...
    do
        ...
        do  
            ...
            do
                # training
                echo Train Task: ${task}-${n_rotation}-${n_demo}-${seed}/${agent} --gpu ${gpu_id} --postfix soft;
                gnome-terminal --wait -- /bin/bash  \
                    ...
                    python train.py --task ${task} --n_demos ${n_demo} --n_rotations ${n_rotation} \
                            ...
                            --gpu_id ${gpu_id} --logging --seed ${seed} --config_file train-mix-soft.yaml --postfix soft; \

                    sleep 30;"
                sleep 1

                # testing
                echo Test Task: ${task}-${n_rotation}-${n_demo}-${seed}/${agent} --gpu ${gpu_id} --postfix soft;
                gnome-terminal --wait -- /bin/bash  \
                    -c "source ~/.bashrc; cd ~/workspace/equivariant_ravens; pwd;   \
                    ...
                    python gripper_test.py --task ${task} --n_demos ${n_demo} --n_rotations ${n_rotation}  \
                            ...
                            --seed ${seed} --entire --config_file train-mix-soft.yaml --disp --postfix soft # --n_steps ${n_tests};  \ 

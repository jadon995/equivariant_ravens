...
class Attention:
    def __init__(self,in_shape,n_rotations,preprocess,device,lite=True,angle_lite=False,init=False):
        ...
        if lite:
          ...
          # self.model = equi_gr_res(in_dim=6,out_dim=1,N=4, middle_dim=(32, 64, 128),init=init).to(self.device)
          self.model = equi_gr_res(in_dim=6,out_dim=1,N=4, middle_dim=(16, 32, 64),init=init).to(self.device)
        else:
          # self.model = dian_res(in_dim=6,out_dim=1,N=4,middle_dim=(32, 64, 128, 256),init=init).to(self.device)
          self.model = equi_gr_res(in_dim=6,out_dim=1,N=4, middle_dim=(64, 128, 256),init=init).to(self.device)
        if angle_lite:
    ...
    def train(self,in_img,p,theta,backprop=True):
        ...
        # index = torch.argmax(output).unsqueeze(dim=0)
        # print('output: ', index.detach().cpu().item(),
        #       ' value: ', output.detach().cpu().numpy()[0, index.detach().cpu().item()],
        #       ' label: ', label.detach().cpu().item(),
        #       ' value: ', output.detach().cpu().numpy()[0, label.detach().cpu().item()],

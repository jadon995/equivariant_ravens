...
from so2_equ_res import SO2ResGroup
...
class Transport:
    ...
    def __init__(self, in_shape, n_rotations, crop_size, preprocess, device, 
        ...
        if network_params['transport']['lite']:
            ...
            #                             init=init, **irrep_kwargs).to(self.device)
            self.model_map = SO2ResGroup(in_dim=6, out_dim=self.n_dim_per_ori, N_out=self.n_ori_align,
                                         middle_dim=(16, 32, 64, 128), init=init, **irrep_kwargs).to(self.device)
            self.model_kernel = SO2ResGroup(in_dim=6, out_dim=self.n_dim_per_ori, N_out=self.n_rotations,
                                         middle_dim=(16, 32, 64, 128), init=init, **irrep_kwargs).to(self.device)
        else:
            ...
            # self.model_kernel = so2_res(in_dim=6, out_dim=3, middle_dim=(32, 64, 128, 256),
                                        # init=init, **irrep_kwargs).to(self.device)
            self.model_map = SO2ResGroup(in_dim=6, out_dim=self.n_dim_per_ori, N_out=self.n_ori_align,
                                         middle_dim=(32, 64, 128, 256), init=init, **irrep_kwargs).to(self.device)
            self.model_kernel = SO2ResGroup(in_dim=6, out_dim=self.n_dim_per_ori, N_out=self.n_rotations,
                                         middle_dim=(32, 64, 128, 256), init=init, **irrep_kwargs).to(self.device)                                        
    ...
    def forward(self, in_img, p, softmax=True, train=True):
        ...
        # TODO Double-check the order of the dimension

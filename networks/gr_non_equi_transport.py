...
import numpy as np
import torch
from gr_convnet import GenerativeResNet
...
class Transport:
    ...
    def __init__(self, in_shape, n_rotations, crop_size, preprocess,device):
        ...
        self.model_map = GenerativeResNet(self.in_type, outdim=self.output_dim).to(self.device)
        self.model_kernel = GenerativeResNet(self.in_type, outdim=self.kernel_dim).to(self.device)

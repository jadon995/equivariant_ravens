...
from gr_non_equi_transport import Transport
from gr_non_equi_attention import Attention
...
class TransporterAgent:
    ...
    def __init__(self, name, task, root_dir, device=1, n_rotations=36, load=False):
        ...
        self.models_dir = os.path.join(root_dir, 'checkpoints_gr', self.name)
    ...
    def train(self, dataset, writer=None):
        ...
        print(f'Train Iter: {step} Loss: {loss0:.4f} {loss1:.8f} time: {time_0:.4f}')
    ...
    def act(self, obs, info=None, goal=None):  # pylint: disable=unused-argument
        ...
        pick_conf = self.attention.forward(img, train=False)
        ...
        p0_xyz = list(p0_xyz)
        p0_xyz[2] = 0.03 + 0.03  # cube is 0.08X0.03 X 0.04
        p0_xyz = tuple(p0_xyz)
        p1_xyz = list(p1_xyz)

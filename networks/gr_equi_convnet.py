...
class EquiGenerativeResNet(torch.nn.Module):
    def __init__(self, n_input_channel=1,
        ...
        self.conv_up_1 = torch.nn.Sequential(OrderedDict([
            ...
            ('dec-e2conv-1', nn.R2Conv(nn.FieldType(self.r2_act, self.l3_c * [self.repr]),
                                       nn.FieldType(self.r2_act, self.l2_c * [self.repr]),
                                       kernel_size=3, stride=1, padding=1, initialize=initialize)),
        ...
        self.conv_up_2 = torch.nn.Sequential(OrderedDict([
            ('dec-e2upsp-2', nn.R2Upsampling(nn.FieldType(self.r2_act, self.l2_c * [self.repr]), 2)),
            ('dec-e2conv-2', nn.R2Conv(nn.FieldType(self.r2_act, self.l2_c * [self.repr]),
                                       nn.FieldType(self.r2_act, self.l1_c * [self.repr]),
                                       kernel_size=3, stride=1, padding=1, initialize=initialize)),
